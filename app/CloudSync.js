...
export default function CloudSync(){
 ...
 async function connectExisting(){const candidate=connectKey.trim();if(candidate.length<32){setStatus('Enter the complete recovery key');return}setBusy(true);try{const result=await request('GET',candidate);if(!result.snapshot?.budget){setStatus('No cloud budget found for that key');return}const cloud={budget:result.snapshot.budget,reconciliation:result.snapshot.reconciliation};const local=snapshot();if(!local.budget){applySnapshot(cloud);localStorage.setItem(SYNC_KEY,candidate);remember(cloud,result.snapshot.updated_at);setStatus('Cloud budget restored. Opening…');setTimeout(()=>location.reload(),250);return}localStorage.setItem(SYNC_KEY,candidate);localStorage.setItem(CLOUD_TIME_KEY,result.snapshot.updated_at||'');localStorage.removeItem(LAST_SYNCED_KEY);setKey(candidate);setConnectKey('');setStatus('Connected. Local budget preserved — restore manually if needed.')}catch(e){setStatus(e.message)}finally{setBusy(false)}}
